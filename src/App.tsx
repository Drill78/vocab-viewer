      ...
      <div className="mx-auto max-w-6xl px-4 py-6">
        <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* 主题切换（默认亮色） */}
            <button className="px-3 py-2 rounded-md border bg-white text-neutral-900 border-neutral-300 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 dark:bg-gh-soft dark:text-gh-text dark:border-gh-border" dark:border-gh-border flex items-center gap-2" onClick={()=> setDark(d=>!d)} title="切换明/暗">
              {dark ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}
              {dark ? "浅色" : "深色"}
            </button>

            <button onClick={exportCSV} className="px-3 py-2 rounded-md border bg-white text-neutral-900 border-neutral-300 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 dark:bg-gh-soft dark:text-gh-text dark:border-gh-border" text-sm flex items-center gap-1">
              <Download className="h-4 w-4" /> 导出
            </button>
            <button onClick={resetAll} className="px-3 py-2 rounded-md border bg-white text-neutral-900 border-neutral-300 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 dark:bg-gh-soft dark:text-gh-text dark:border-gh-border" text-sm flex items-center gap-1">
          ...
          <div className="rounded-2xl border p-4 shadow-soft bg-white dark:bg-gh-soft dark:border-gh-border">
            ...
            <div className="grid gap-3 md:grid-cols-3">
              ...
              <div className="flex items-center gap-2 md:col-span-2">
                <input placeholder="或粘贴 CSV 直链（GitHub Raw / 公网直链）" ref={urlRef} className="flex-1 w-full px-3 py-2 rounded-md border bg-white text-neutral-900 placeholder-neutral-500 border-neutral-300 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 dark:bg-gh-soft dark:text-gh-text dark:placeholder-neutral-500 dark:border-gh-border"/>
                <button className="px-3 py-2 rounded-md border bg-white text-neutral-900 border-neutral-300 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 dark:bg-gh-soft dark:text-gh-text dark:border-gh-border" text-sm flex items-center gap-1" onClick={()=> handleURL(urlRef.current?.value||"")}>
        ...
        <section className="mt-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="flex-1 flex items-center gap-2">
            <div className="relative flex-1">
              <input className="w-full px-9 py-2 rounded-md border bg-white text-neutral-900 placeholder-neutral-500 border-neutral-300 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 dark:bg-gh-soft dark:text-gh-text dark:placeholder-neutral-500 dark:border-gh-border" placeholder="搜索中文/英文/释义/类别/标签…" value={query} onChange={(e)=> setQuery(e.target.value)} />
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-neutral-400"/>
            </div>
            <select className="px-3 py-2 rounded-md border bg-white text-neutral-900 border-neutral-300 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 dark:bg-gh-soft dark:text-gh-text dark:border-gh-border" value={category} onChange={(e)=> setCategory(e.target.value)}>
              {allCategories.map(c => <option key={c} value={c}>{c==="all"?"全部类别":c}</option>)}
            </select>
            <select className="px-3 py-2 rounded-md border bg-white text-neutral-900 border-neutral-300 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 dark:bg-gh-soft dark:text-gh-text dark:border-gh-border" value={tag} onChange={(e)=> setTag(e.target.value)}>
              {allTags.map(t => <option key={t} value={t}>{t==="all"?"全部标签":t}</option>)}
            </select>
            <select className="px-3 py-2 rounded-md border bg-white text-neutral-900 border-neutral-300 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 dark:bg-gh-soft dark:text-gh-text dark:border-gh-border" value={sortBy} onChange={(e)=> setSortBy(e.target.value as any)}>
          ...
          <section className="mt-6">
            <div className="rounded-2xl border p-6 shadow-soft bg-white dark:bg-gh-soft dark:border-gh-border">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                  <button className="px-3 py-2 rounded-md border bg-white text-neutral-900 border-neutral-300 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 dark:bg-gh-soft dark:text-gh-text dark:border-gh-border" text-sm flex items-center gap-1" onClick={()=> setShowBack(s=>!s)}>
              ...
              {current ? (
                <motion.div key={current.id + String(showBack)} initial={{ rotateY: 180, opacity: 0 }} animate={{ rotateY: 0, opacity: 1 }} transition={{ duration: 0.35 }} className="grid gap-4 text-center select-none">
                  ...
                  <div className="flex items-center justify-center gap-3 pt-2">
                    <button className="px-3 py-2 rounded-md border bg-white text-neutral-900 border-neutral-300 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 dark:bg-gh-soft dark:text-gh-text dark:border-gh-border" text-sm" onClick={prevCard}>上一张</button>
                    <button className="px-3 py-2 rounded-md border bg-white text-neutral-900 border-neutral-300 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 dark:bg-gh-soft dark:text-gh-text dark:border-gh-border" text-sm" onClick={nextCard}>下一张</button>
                  </div>
                </motion.div>
              ) : (
