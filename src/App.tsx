...
import { Moon, Sun, Shuffle, RotateCcw, Eye, EyeOff, Info, Download, Link as LinkIcon, Search, Globe, Star, FolderOpen } from "lucide-react";
...
const coerceUiLang = (x: any): UILang => (x === "en" ? "en" : "zh");
const coerceLangMode = (x: any): LangMode =>
  (x === "en" || x === "zh" || x === "both") ? (x as LangMode) : "both";
...
type LibraryItem = { title: string; path: string; description?: string };

export default function App() {
  const [data, setData] = useState<Vocab[]>(load<Vocab[]>(STORAGE_KEY, []));
  const [uiLang, setUiLang] = useState<UILang>(() => coerceUiLang(localStorage.getItem(PREF_KEY+":uiLang")));
  const [lang, setLang] = useState<LangMode>(() => coerceLangMode(localStorage.getItem(PREF_KEY+":lang")));
  ...
  // 修复历史上写入的异常值（如果存在）
  useEffect(()=>{
    const u = localStorage.getItem(PREF_KEY+":uiLang");
    const l = localStorage.getItem(PREF_KEY+":lang");
    const u2 = coerceUiLang(u);
    const l2 = coerceLangMode(l);
    if(u !== u2){ setUiLang(u2); save(PREF_KEY+":uiLang", u2); }
    if(l !== l2){ setLang(l2); save(PREF_KEY+":lang", l2); }
  }, []);
  ...
  async function handlePublicPick() {
    setShowLibrary(true);
    try{
      // 读取 /data/library.json；若失败则使用内置候选
      const res = await fetch("/data/library.json", { cache: "no-store" });
      if (!res.ok) throw new Error("no library");
      const list = await res.json() as LibraryItem[];
      setLibrary(Array.isArray(list) ? list : []);
    } catch {
      setLibrary([
        { title: "逻辑学（样例）", path: "/data/vocab.csv", description: "基础样例，演绎/归纳等" },
        { title: "哲学术语（样例）", path: "/data/philosophy.csv", description: "常见哲学术语" },
        { title: "课堂通用词（样例）", path: "/data/classroom.csv", description: "课堂常用表达" },
      ]);
    }
  }
  async function loadFromPublicPath(p: string) {
    try{
      setLoading(true); setProgress(10);
      ...
      const out = await parseCSVText(text); setProgress(95);
      setData(out);
      toast.success(`${uiLang==='zh' ? '已载入' : 'Loaded'} ${p}: ${out.length}`);
    }catch(e){
      toast.error((uiLang==='zh' ? '载入失败：' : 'Failed: ') + p);
    }finally{
  ...
  const t = (key: keyof typeof i18n.zh) => (i18n[uiLang] && i18n[uiLang][key]) ?? i18n.zh[key];
  ...
  return (
    <ErrorBoundary>
      <div className="min-h-screen bg-white dark:bg-[#0d1117] text-neutral-900 dark:text-[#c9d1d9] transition-colors">
            ...
            <div className="rounded-2xl border p-3 sm:p-4 shadow bg-white dark:bg-[#161b22] dark:border-[#30363d]">
              ...
              <div className="grid gap-3 md:grid-cols-3">
                ...
                <div className="flex items-center gap-2 md:col-span-2">
                  ...
                  <button className="h-10 px-3 py-2 rounded-md border text-sm flex items-center gap-1" onClick={handlePublicPick}>
                    <FolderOpen className="h-4 w-4" /> 从公共库中选择
                  </button>
                </div>
        ...
        {/* 公共库弹窗 */}
        {showLibrary && (
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40" onClick={()=> setShowLibrary(false)}>
            <div className="w-full sm:max-w-lg bg-white dark:bg-[#161b22] dark:text-[#c9d1d9] rounded-t-2xl sm:rounded-2xl border dark:border-[#30363d] p-4 sm:p-5 shadow-xl" onClick={(e)=> e.stopPropagation()}>
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-base sm:text-lg font-semibold flex items-center gap-2"><FolderOpen className="h-4 w-4"/> 从公共库中选择</h3>
                <button className="px-2 py-1 rounded-md border text-sm" onClick={()=> setShowLibrary(false)}>关闭</button>
              </div>
              <div className="space-y-2 max-h-[60vh] overflow-auto">
                {library.map((it, idx) => (
                  <button key={idx} className="w-full text-left p-3 rounded-lg border hover:bg-neutral-50 dark:hover:bg-[#0d1117]" onClick={()=> loadFromPublicPath(it.path)}>
                    <div className="font-medium">{it.title}</div>
                    <div className="text-xs text-neutral-500">{it.path}</div>
                    {it.description && <div className="text-xs mt-1">{it.description}</div>}
                  </button>
                ))}
                {!library.length && <div className="text-sm text-neutral-500">没有可用条目</div>}
              </div>
            </div>
          </div>
        )}
